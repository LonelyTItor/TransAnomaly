...
def _transformations_experiment(dataset_load_fn, dataset_name, single_class_ind, gpu_q):
    ...
    if dataset_name in ['cats-vs-dogs']:
        ...
        # what is the parameter of n,k : depth and wide-factor
        n, k = (10, 4)
    ...
    mdl.compile('adam',
                ...
                ['acc'])
    print('----------2-----------')
    # select the value only contains the only one idx
    x_train_task = x_train[y_train.flatten() == single_class_ind]
    # create the training pairs (img, and transform types)
    transformations_inds = np.tile(np.arange(transformer.n_transforms), len(x_train_task))
    # then we create the transform batch (St) constructed
...
if __name__ == '__main__':
    ...
    # Here we set the number of the gpus
    N_GPUS = 1

    # Also the multi-processor
    man = Manager()
    q = man.Queue(N_GPUS)
